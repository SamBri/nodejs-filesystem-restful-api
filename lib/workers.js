...
workers.performCheck = function(originalCheckData){
  ...
  // Mark that the outcome has not been sent here.
  ...
  var path = parsedUrl.path; //using path not pathname
...
// Process the check outcome and update the check data as needed, triggger an alert
//Special logic for accomodating a chheck that has never been tested before
...
workers.log = function(originalCheckData,checkOutcome,state,alertWarranted,timeOfCheck){
 ...
 var logString = JSON.stringify(logData,getCircularReplacer());

 // Determine te name of the log file
...
workers.logRotationLoop = function(){
  setInterval(function(){workers.rotateLogs();}, 1000 * 60 * 60 * 24);
};
...
workers.init = function(){
 ...
 workers.gatherAllChecks();
  ...
  // call the compression loop as logs will be compressed later on
